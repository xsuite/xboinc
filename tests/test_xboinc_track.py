...
subprocess.call('clang main.c -o xboinc_executable -lm', shell=True)
...
# run xboinc tracker
subprocess.call('./xboinc_executable', shell=True)
...
assert np.all(particles_xboinc.at_turn == 1000), "Unexpected survivals (particles)"
assert sim_state.i_turn == 1000, "Unexpecteds survival (sim_state)"

# Testing results with xtrack
line_xtrack  = xt.Line.from_dict(input_data['line'])
line_xtrack.build_tracker()
line_xtrack.track(particles, num_turns=1000)

assert np.array_equal(particles.at_turn, particles_xboinc.at_turn), "Fail to match xtraxk: survivals are not equal"
